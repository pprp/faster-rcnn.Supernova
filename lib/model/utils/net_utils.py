...
def vis_detections_split(im, class_name, dets, thresh=0.001,image_name=""):
    """Visual debugging of detections."""
    _,a = image_name.split('/')
    name,_ = a.split('.')
    f = open("/media/learner/Passport/dpj/faster-rcnn.pytorch/submit.csv",'a')
    outtxt = ""+name
    for i in range(np.minimum(3, dets.shape[0])):
        ...
        if score > thresh:
            ...
            xcenter = np.round((bbox[0]+bbox[2])/2)
            ycenter = np.round((bbox[1]+bbox[3])/2)
            tmpx,tmpy=xcenter,ycenter
            outtxt = outtxt + ","+str(int(xcenter))+","+str(int(ycenter))    
        else:
            xcenter,ycenter = tmpx,tmpy
            print("error < thresh, name: %s %d \n" % (name,score))
            outtxt = outtxt + ","+str(int(xcenter))+","+str(int(ycenter))    
    appendText=""
    if class_name == "havestar":
        appendText = ",1"
    else:
        appendText = ",0"
    outtxt = outtxt + appendText + "\n"
    f.write(outtxt)
    f.close()
    return im

def vis_detections(im, class_name, dets, thresh=0.001,image_name="",tag=True):
    if not tag:
        return im
    """Visual debugging of detections."""
    _,a = image_name.split('/')
...
def _smooth_l2_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    sigma_2 = sigma# ** 2
    box_diff = bbox_pred - bbox_targets
    in_box_diff = bbox_inside_weights * box_diff
    abs_in_box_diff = torch.abs(in_box_diff)
